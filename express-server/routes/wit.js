...
let process = (witData) => {
    ...
    if ('greetings' in witData && !('bye' in witData)) {
        let num = Math.random()
        if (num < 0.25) return "Hello!"
        else if (num < 0.5) return "What up dawg"
        else if (num < 0.75) return "yo yo"
        else return "my duude"
        ...
        if (intent == 'get_skills') {
            ...
            if ('Itential' in witData) {
                ...
                return "I am currently interning at Amazon this summer as a SWE. Previously, I have interned at Itential, a network automation company, and UPS. Type 'Itential' or 'UPS' to learn more about these internships!"
            }
            ...
            return "Some of my hobbies include playing the piano, playing tennis and exploring the outdoors/photography."
            ...
            let num = Math.random()
            if (num < 0.5) return "aww you're so kind :)"
